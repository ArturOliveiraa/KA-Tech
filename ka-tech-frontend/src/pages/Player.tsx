...
    const [showLessonQuiz, setShowLessonQuiz] = useState(false); // NOVO ESTADO ADICIONADO PARA AULA
...
                        <div style={{ display: 'flex', gap: '12px' }}>
                            {/* BOTÃO DO QUIZ DA AULA ADICIONADO AQUI */}
                            <button 
                                onClick={() => setShowLessonQuiz(true)}
                                style={{ 
                                    background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)', // Azul para diferenciar
                                    color: '#fff', 
                                    border: 'none', 
                                    padding: '10px 16px', 
                                    borderRadius: '8px', 
...
                             {isMobile && (
                                <>
                                    <button onClick={() => setShowLessonQuiz(true)} style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 700 }}>
                                        <IconBrain /> Aula
                                    </button>
...
            {/* MODAL DO QUIZ DO MÓDULO - Sobrepõe tudo quando ativado */}
...
            )}
            ...
            {/* NOVO: MODAL DO QUIZ DA AULA ESPECÍFICA */}
            {showLessonQuiz && activeLessonId && (
                <div style={{ 
                    ...
                    <div style={{ width: '100%', maxWidth: '900px' }}>
                        <QuizPlayer 
                            lessonId={activeLessonId} 
                            onExit={() => setShowLessonQuiz(false)} 
                        />
                    </div>
                </div>
            )}
