...
export default function ContentManagement() {
  ...
  // --- FORMA LIMPA DE EXCLUSÃO (DEPENDENTE DO CASCADE NO BANCO) ---
  const handleDeleteCourse = async (id: number) => {
    if (!window.confirm("Isso apagará o curso e TODO o progresso de todos os alunos. Deseja continuar?")) return;
    
    try {
      // Com o ON DELETE CASCADE configurado no banco, deletar o curso remove 
      // automaticamente matrículas, aulas, tags e insígnias.
                  ...
                  {courses.map(course => (
                    <div key={course.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '25px', background: 'linear-gradient(145deg, #020617, #09090b)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.03)', transition: '0.3s' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <button onClick={() => handleDeleteCourse(course.id)} style={{ background: 'rgba(239, 68, 68, 0.1)', color: '#f87171', border: '1px solid rgba(239, 68, 68, 0.2)', padding: '10px 14px', borderRadius: '10px', cursor: 'pointer', fontWeight: 700, fontSize: '0.75rem' }}>Excluir</button>
                      </div>
                    </div>
                  ))}
                </div>
