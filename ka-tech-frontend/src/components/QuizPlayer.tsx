...
export function QuizPlayer({ courseId, onExit }: QuizPlayerProps) {
  ...
  // 1. CARREGAMENTO INICIAL COM TRATAMENTO DE ERROS
  useEffect(() => {
    async function init() {
      try {
        setLoading(true);
        setNotFound(false); // Reset do estado ao trocar de curso

        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return;

        // Buca o quiz mais recente para evitar o erro PGRST116 (múltiplas linhas)
        ...
        // Se houver erro ou o array vier vazio, activa a tela de "Não Encontrado"
        ...
        // Verifica se o utilizador já concluiu este quiz
        ...
        if (attempt) {
          ...
          // Prepara o jogo e baralha as opções
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          setIsAnswerChecked(true); // Trava a resposta ao zerar o tempo
          return 0;
        }
        return prev - 1;
      });
  ...
  if (loading) return (
    <div className="flex-center" style={{ height: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8b5cf6' }}>
      <Loader2 className="animate-spin" size={32} />
    </div>
  );

  // MENSAGEM: SEM QUIZ (Agora garantida)
  if (notFound) {
    return (
      <div className="qp-full-screen">
        <div className="qp-content-card">
          <BookOpen size={64} color="#8b5cf6" style={{ marginBottom: '20px' }} />
          <h1 className="qp-status-title">Avaliação indisponível</h1>
          <p className="qp-status-text">
            Este treinamento ainda não possui uma avaliação cadastrada em nosso sistema.
          </p>
          <button onClick={onExit} className="qp-btn-back">
            Voltar ao Curso
          </button>
        </div>
        ...
        <style>{`
            .qp-full-screen {
                position: fixed;
                inset: 0;
                background: #020617; /* Fundo bem escuro para contraste */
                display: flex;
                align-items: center;
            ...
            .qp-status-text {
                color: #94a3b8 !important; /* Cinza claro legível */
                font-size: 1.1rem;
                line-height: 1.6;
                margin-bottom: 35px;
                font-weight: 500;
  ...
  return (
    <div className="quiz-container">
      <style>{`
        ...
        .btn-primary:hover:not(:disabled) { transform: translateY(-3px); filter: brightness(1.1); }
      ...
      <div className="glass-panel">
        ...
        <div className="qp-footer" style={{ padding: '35px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.25)', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {!isAnswerChecked ? (
            ...
            <button className="btn-primary" onClick={handleNextQuestion} style={{ background: currentQuestionIndex + 1 === quiz.questions.length ? '#10b981' : '#8b5cf6', display: 'flex', alignItems: 'center', gap: '12px', justifyContent: 'center' }}>
              {currentQuestionIndex + 1 === quiz.questions.length ? (savingResult ? "A gravar..." : "Ver Resultado") : "Próxima Questão"}
              <ArrowRight size={24} />
            </button>
          )}
        </div>
